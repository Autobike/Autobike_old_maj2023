 ...
 * 
 * Captures an outgoing packet and stores it as a Pascal string. Pascal strings
 * are used instead of C strings since C strings cannot hold null bytes.
 * 
 ...
 * @author Ossian Eriksson
*/
static void rxValue(mc_values *v) {
    values = *v;
}

/**
 * Initializes the VESC. You should call this function before calling any other
 ...
 * 
 * @param bytes The recieved bytes
 ...
 * 
 * NOTE: The returned string is a Pascal string, make sure to select this
 * option when configuring the return type from within the "Call Library
 ...
 * Function Node" in LabVIEW.
 * 
 * @param RPM Desired motor RPM [rpm]
 * 
 * @return UART package as Pascal string
 * 
 * @see https://www.ni.com/docs/en-US/bundle/labview/page/lvexcodeconcepts/array_and_string_options.html
 ...
 * Gets current values
 * 
 * @param RPM Output for motor RPM [rpm]
 * @param currentIn Output for motor controller input current [A]
 * @param currentMotor Output for motor current [A]
 * @param tempMOS Output for MOSFET temperature [°C]
*/
